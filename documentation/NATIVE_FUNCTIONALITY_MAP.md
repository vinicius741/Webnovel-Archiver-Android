...
- Swipe-between-tabs: when two or more tabs exist (including the synthetic All/Unassigned tabs), the story grid is rendered as a `ViewPager2` with one page per tab, so a horizontal swipe switches the active tab. The tab bar and pager stay two-way synced — a swipe updates the active-tab indicator and persists the selection, and a tab-bar tap animates the pager to that page. The single-tab case keeps the original single-grid layout.
