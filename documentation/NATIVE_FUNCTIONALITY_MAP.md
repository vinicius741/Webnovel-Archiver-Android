...
This map was created during the React Native to native Kotlin migration.
...
## Sources
- Network requests use the React Native-compatible mobile user agent, HTML/image accept headers, English accept-language, Scribble Hub URL-encoded AJAX form posts, per-host Scribble Hub rate limiting, and retry handling for Scribble Hub 403/429 responses.
- Source chapter titles are normalized with React Native-compatible cleanup for trailing overflow markers, relative "time ago" suffixes, and absolute date suffixes.
...
- Scribble Hub support:
  ...
  - URL fallback story IDs use JavaScript `encodeURIComponent`-compatible percent encoding to match the React Native provider.
  - Chapter ID from `/chapter/{id}`.
...
- Queue controls preserve React Native-compatible status transitions: pause clears scheduled retry timers, resume keeps existing error metadata while clearing retry timers, cancel records cancelled error metadata, and manual retry resets only failed jobs.
...
- Default sentence-removal list matches the React Native `default_sentence_removal.json` anti-piracy cleanup phrases.
...
- Quick separator rule builder generates repeated-character cleanup patterns matching the React Native helper behavior.
...
- Reader uses the React Native-compatible undownloaded chapter fallback message.
...
- Persisted chapter filter preference matching the React Native `all`, `hideNonDownloaded`, and `hideAboveBookmark` modes.
...
- EPUB cover, details, table-of-contents, chapter XHTML, and CSS mirror the React Native content processor structure, including cover placeholders, description paragraphs, tag chips, and reader-friendly styles.
...
- EPUB split size is clamped to the React Native range of 10-1000 chapters, and generated EPUB filenames use bounded sanitized story-title bases with chapter range suffixes preserved.
...
- JSON metadata backup export rejects empty libraries and oversized JSON payloads above 50 MB with React Native-compatible messages.
...
- Full ZIP backup export/restore using a React Native-compatible manifest with `config`, scrubbed story file paths, a `chapterFiles` index, settings, tabs, cleanup rules, display preferences, chapter filter preference, TTS settings/session, library, and chapter files.
...
- Full ZIP backup chapter file entries include React Native-compatible `storyId`, `chapterId`, `chapterIndex`, `title`, and portable `novels/{encodedStoryId}/{0000}_{encodedChapterId}.html` paths.
...
- Full ZIP restore reports a React Native-compatible missing-manifest error before destructive restore.
...
- Persisted preference loading normalizes invalid, high out-of-range, or legacy partial settings back to React Native-compatible bounds/defaults for global downloads, per-source downloads, TTS, chapter filter mode, active theme, and fold layout mode.
...
- Persisted fold layout mode (`auto`, `cover`, `inner`) for parity with the React Native foldable preference.
...
- Native unit tests:
  ...
  - `SentenceRemovalPlanningTest` verifies React Native-compatible sentence trimming, blank/duplicate rejection, insertion order, index editing, and deletion.
  - `DefaultCleanupTest` verifies the native default sentence-removal list matches the React Native default asset shape.
  - `ReaderContentRendererTest` verifies native reader HTML wrapping, escaped titles, viewport metadata, font sizing, responsive image styling, and the React Native-compatible undownloaded chapter fallback message.
  ...
  - `SourceProviderTest` also verifies Scribble Hub fallback story ID encoding matches the React Native provider.
  - `SourceProviderTest` also verifies React Native-compatible chapter title cleanup for overflow markers, relative timestamps, and absolute dates.
  - `SourceUrlValidationTest` verifies import gating for supported story URLs and rejection of chapter/non-story URLs.
  - `BrowserUrlPlanningTest` verifies React Native-compatible browser URL resolution and Google auth external-routing detection.
  - `TabPlanningTest` verifies tab creation, rename validation, delete reordering, and move ordering behavior.
  - `NetworkRequestsTest` verifies React Native-compatible fetch/form request headers and Scribble Hub AJAX form body shape.
  ...
  - `DownloadRangeSelectionTest` verifies explicit range, bookmark-forward range, count-based range, and invalid range handling with React Native-compatible validation messages.
  ...
  - `StoryActionGuardsTest` verifies archived snapshots cannot sync or queue downloads and use React Native-compatible guard messages.
  ...
  - `FullBackupPathsTest` verifies React Native-compatible full-backup chapter path encoding and filename generation.
  ...
  - `FullBackupRestorePlanningTest` verifies full-backup restore metadata scrubbing, chapter file re-linking, downloaded-count recomputation, and React Native-compatible restore summary counts.
  ...
  - `StoryBookmarkPlanningTest` verifies bookmark toggling and React Native-compatible EPUB range advancement when `startAfterBookmark` is enabled.
  ...
  - `SettingsValidationTest` verifies React Native-compatible settings bounds for download concurrency, delay, EPUB size, TTS pitch/rate, and TTS chunk size.
