...
## 🚫 DEVICE POLICY: NEVER USE PHYSICAL PHONE BY DEFAULT 🚫

### THE ONE RULE THAT MATTERS MOST:
**NEVER target, touch, or run commands on the owner's physical phone unless they explicitly request it in their CURRENT message.** Vague references, past permission, or the phone being connected do NOT count.

*   **Default Target:** Always use the simulator (`webnovel_api36` AVD) for all builds, installs, launches, and tests.
*   **Off-Limits Actions:** You must NOT install/uninstall apps, deploy APKs, launch the app, send notifications, or target the phone's serial with `adb` commands (even if it appears in `adb devices`—ignore it).
*   **Explicit Requests Only:** Each deploy requires fresh, explicit instructions in the *current* message (e.g., "install on my phone"). If you are not 100% certain, you MUST use the simulator.
