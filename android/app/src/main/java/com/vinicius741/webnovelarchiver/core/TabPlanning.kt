...
object TabPlanning {
    ...
    private fun assignOrders(tabs: List<Tab>): MutableList<Tab> =
        tabs.mapIndexed { index, tab -> tab.copy(order = index) }.toMutableList()

    fun create(tabs: List<Tab>, rawName: String, id: String, createdAt: Long): MutableList<Tab> {
    ...
    fun rename(tabs: List<Tab>, id: String, rawName: String): MutableList<Tab> {
        val name = rawName.trim()
        val normalized = normalizeOrders(tabs)
        if (name.isBlank()) return normalized
    ...
    fun delete(tabs: List<Tab>, id: String): MutableList<Tab> =
        normalizeOrders(tabs.filterNot { it.id == id })

    fun move(tabs: List<Tab>, fromIndex: Int, toIndex: Int): MutableList<Tab> {
        val normalized = normalizeOrders(tabs)
