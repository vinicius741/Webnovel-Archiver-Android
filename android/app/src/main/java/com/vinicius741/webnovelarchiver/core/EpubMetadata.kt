...
object EpubMetadata {
    fun opf(story: Story, chapters: List<Chapter>, cover: EpubCoverMetadata?): String {
    ...
    fun ncx(story: Story, chapters: List<Chapter>): String {
        val frontMatter = listOf(
            ...
            "<navPoint id=\"navPoint-${item.first}\" playOrder=\"${index + 1}\"><navLabel><text>${xml(item.second)}</text></navLabel><content src=\"${xml(item.third)}\"/></navPoint>"
        }.joinToString("")
        val chapterNav = chapters.mapIndexed { index, chapter ->
            "<navPoint id=\"navPoint-${index + 1}\" playOrder=\"${index + 4}\"><navLabel><text>${xml(chapter.title)}</text></navLabel><content src=\"chapter_${index + 1}.xhtml\"/></navPoint>"
        }.joinToString("")

        return """<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE ncx PUBLIC "-//NISO//DTD ncx 2005-1//EN" "http://www.daisy.org/z3986/2005/ncx-2005-1.dtd"><ncx xmlns="http://www.daisy.org/z3986/2005/ncx/" version="2005-1"><head><meta name="dtb:uid" content="urn:webnovel:${xml(story.id)}"/><meta name="dtb:depth" content="1"/><meta name="dtb:totalPageCount" content="0"/><meta name="dtb:maxPageNumber" content="0"/></head><docTitle><text>${xml(story.title)}</text></docTitle><navMap>$frontMatter$chapterNav</navMap></ncx>"""
    }
