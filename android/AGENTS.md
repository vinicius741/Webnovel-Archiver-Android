...
- `source/` owns network/source providers and Cloudflare detection/solving; `sync/`, `download/`, `epub/`, `cleanup/`, `tts/`, and `ai/` own their respective engines and planning logic. The `ai/` package is the OpenRouter layer (client on a plain OkHttp stack — deliberately not the Cloudflare-intercepted `NetworkClient` — plus description and cover-art planning/engines); cover generation runs as process-lifetime background jobs (`AiCoverJobCoordinator` + `AiCoverForegroundService`), persisting each stage's draft before announcing it. Its per-novel UI is `feature/ai/AiControlsScreen.kt` (reached from Details' "More options" menu), which generates drafts and only persists on Apply — see `docs/ai/`.
