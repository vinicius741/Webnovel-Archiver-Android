...
- `domain/model/` contains persisted/application models; `domain/archive/`, `domain/story/`, and `domain/metrics/` contain pure domain rules (including publication-status lifecycle and per-novel metric-trend snapshot planning/retention). `domain/metrics/MetricSnapshotPlanning.kt` is the single owner of same-day coalescing + downsampling + series extraction for the Trends feature.
- `feature/details/TrendsScreen.kt` (`showTrends`) is the per-novel metric Trends sub-screen reached from the detail page's score row, Patreon card, and overflow menu; charts are built in `feature/details/TrendChart.kt` via MPAndroidChart (added on JitPack, see `settings.gradle`).
