...
- `app/` owns Android lifecycle and process-wide dependency wiring (`WebnovelArchiverApp`, `MainActivity`, `AppContainer`, `RepositoryStartup`). `WebnovelArchiverApp` constructs the shared container on its IO scope; `MainActivity` shows startup UI and awaits it before wiring engines. `AppContainer` holds the single process-wide instances of repository, network, sync/epub/tts engines.
...
- `feature/browser/` launches third-party sites in a browser-powered Custom Tab and receives import URLs through `MainActivity`; do not move third-party OAuth back into a WebView. It also hosts `CloudflareSolveActivity`, a narrow in-app WebView that solves interactive Cloudflare challenges in the shared `android.webkit.CookieManager` session. `source/network/CloudflareBypassInterceptor.kt` normally uses OkHttp, but after a challenge it switches that host to a persistent, serialized Chromium-rendered transport for the active passage window instead of replaying a browser token through a different TLS fingerprint. `SourceReliabilityCoordinator` owns the process-wide request budget, adaptive pacing, cooldown, and manual-verification circuit shared by sync and downloads. That CF path is distinct from OAuth — third-party login/credential flows still go through Custom Tabs, never the in-app WebView.
- `feature/` groups browser, cleanup, details, downloads, library, player, reader, settings, updates, AI controls, and shared story-action UI by user-facing flow.
...
- `domain/model/` contains persisted/application models; `domain/archive/`, `domain/story/`, and `domain/metrics/` contain pure domain rules (including publication-status/source-availability lifecycles and per-novel metric-trend snapshot planning/retention). `domain/metrics/MetricSnapshotPlanning.kt` is the single owner of same-day coalescing + downsampling + series extraction + trend direction for the Trends feature; `domain/metrics/TrendAxisPlanning.kt` owns the data-fitted chart Y-range math.
- `feature/details/TrendsScreen.kt` (`showTrends`) is the per-novel metric Trends sub-screen reached from the detail page's score row, Patreon card, and overflow menu; charts are built in `feature/details/TrendChart.kt` via MPAndroidChart (added on JitPack, see `settings.gradle`) with Y axes fitted to the data (not pinned to the full metric domain) and a fixed chart height set by the screen. The details header also patches a small trend arrow beside the score once the history loads (`DetailsHeader.observeScoreTrend`).
- `feature/ai/AiControlsScreenState.kt` owns activity-lifetime drafts and per-story cover disclosure state. Its `AiControlsBinding` holds only the attached AI Controls views; `screen()` and activity destruction clear it. Message-only AI updates use `updateAiControlsProgress`; job transitions and new results rebuild controls. Keep background coordinators independent of these Views.
- `feature/story/ManualStorySync.kt` owns the common manual lookup/fetch/download-plan sequence. URL import and existing-story actions retain their own UI callbacks. Bulk Updates has separate orchestration.
...
- `perf/` owns the debug-gated performance recorder (`PerfRecorder`, session state machine, FrameMetrics collector; see `docs/architecture/performance-tracking.md`). It activates only when a launch intent carries `perf_session 1` (the `scripts/perf/perf_runner.py` host runner does this); every hook call site is behind `BuildConfig.DEBUG`, so release behavior is unchanged. Don't route new timings around it — add events via `PerfInstrumentation` when a screen or phase needs measuring.
...
- `feature/player/` is the podcast-style TTS player UI: the app-level mini-player bar (`attachTtsMiniPlayer`, pinned above every screen while a session is loaded) and the Now Playing screen (`showPlayer`, `AppRoute.Player`). Playback state comes from `TtsEngine.playbackState`; per-story resume positions persist in `tts_positions.json` (see `docs/tts/tts-podcast-player.md`).
- `source/` owns network/source providers and Cloudflare detection/solving; `sync/`, `download/`, `epub/`, `cleanup/`, `tts/`, and `ai/` own their respective engines and planning logic. The `ai/` package contains the OpenRouter and TypeSafe layers (client on a plain OkHttp stack — deliberately not the Cloudflare-intercepted `NetworkClient` — plus description, cover-art, and chapter-rewrite planning/engines); cover and chapter-rewrite generation run as process-lifetime background jobs (`AiCoverJobCoordinator`/`AiChapterRewriteJobCoordinator` + their foreground services), persisting each result before announcing it. Chapter polish rewrites downloaded chapters into verified local variants resolved for Reader/TTS by `feature/reader/ChapterContentResolver.kt` (source files stay immutable). The per-novel AI UI is `feature/ai/AiControlsScreen.kt` (reached from Details' "More options" menu), which saves cover experiments locally and changes the displayed cover only on Use this cover — see `docs/ai/`.
...
- Automatic cover context uses `CoverEvidenceSelector` and `TypeSafeCoverClient` to score downloaded passages with a separate device-local TypeSafe key. `CoverEvidencePlanning` owns chunking and selection. `CoverEvidenceCache` stores disposable content-addressed score responses under app cache, outside library/backup storage; never put source text or keys in it. Manual cover chapter selections bypass TypeSafe.
...
Tokens: `library`, `queue` (download manager), `settings`, `notifications`, `aisettings`, `aicontrols` (first story's AI Controls), `updates`, `followupdates`, `reader`, `details`, `addstory`.

- No-arg screens (`library`, `queue`, `settings`, `notifications`, `aisettings`, `updates`, `followupdates`, `addstory`) need nothing else.
...
**Dev library report (restore/import verification).** Adding `--es dev_library_report 1` to a cold start makes the debug app write what it actually hydrated from storage to `cache/dev_library_report.json` (readable via `run-as <pkg> cat`): library size, a sha256 over the loaded story-id list, per-tab counts, downloaded-chapter total, and any documents the storage layer quarantined as corrupt. This is the deterministic way to verify a backup restore/library seed (used by the `restore-library-backup` skill) — a matching `storyIdsSha256` proves the app loaded exactly the intended stories in order, and a non-empty `storageIssues` exposes a malformed seeded file that would otherwise vanish silently. Planning lives in `app/DevLibraryReportPlanning.kt`; same `BuildConfig.DEBUG` gating, no-op in release. Poll for the file rather than fixed-sleeping — hydrating a large library can take several seconds.

**Dev full-backup restore.** Adding `--es dev_restore_full_backup <path-relative-to-cacheDir>` to a cold start restores a `webnovel_full_backup_*.zip` staged inside the app's own cache (adb push + `run-as cp` to `cache/dev_restore_source.zip`) through the production `FullBackupRestorer` pipeline — extraction, manifest/ZIP-index validation, staging, verification, atomic root swap, and repository refresh, identical to the in-app Settings picker but without the SAF file picker. The path is resolved by `app/DevRestorePlanning.kt`, which rejects absolute and `..` paths; the staged zip is deleted after the attempt so relaunching never re-restores, and the dev library report is written automatically afterwards. Debug-only, no-op in release. Expect minutes, not seconds: a 113 MB / 10,808-chapter ZIP measured ~7-8 minutes on the emulator. Expectations for verification come from `scripts/full_backup_expectations.py <zip>`.

### Local performance tracking (perf-suite)

`scripts/perf/perf_runner.py run --suite short` drives repeatable emulator scenarios (cold library/settings/queue/details/reader, warmed navigation, scrolls) against the debug app and writes a JSON + Markdown report with in-app monotonic timing, frame/jank stats, memory snapshots, and crash/ANR/StrictMode evidence into gitignored `perf-runs/`. Comparisons against explicitly saved baselines are advisory regression signals — debug/emulator numbers are never release or device claims. The app side is the debug-gated `perf/` recorder (design + metric definitions: `docs/architecture/performance-tracking.md`; agent workflow: the `perf-suite` skill). Baselines are dataset-fingerprint-locked and created only via `baseline save`; `selftest` runs the pure-logic tests without a device.
