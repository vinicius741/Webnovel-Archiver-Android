...
| `performance-tracking.md` | Local performance tracking for the debug app: debug-gated in-app recorder (`perf/` package), host runner (`scripts/perf/perf_runner.py`) with cold-start/navigation/scroll scenarios, frame/jank/memory evidence, advisory baselines, metric definitions, methodology, and caveats. |
