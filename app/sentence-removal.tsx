...
function getDerivedPattern(draft: RuleDraft): {
  pattern: string;
  flags: string;
} {
...
export default function SentenceRemovalScreen() {
  ...
  const handleSaveSentence = () => {
    ...
    if (
      existingIndex !== -1 &&
      (editingSentenceIndex === null || existingIndex !== editingSentenceIndex)
    ) {
  ...
  const derivedPattern = useMemo(
    () => getDerivedPattern(ruleDraft),
    [ruleDraft],
  );

  const effectivePattern =
    ruleDraft.mode === "quick" ? derivedPattern.pattern : ruleDraft.pattern;
  const effectiveFlags =
    ruleDraft.mode === "quick" ? derivedPattern.flags : ruleDraft.flags;
  ...
  const handleSaveRule = () => {
    if (!ruleValidation.valid) {
      showAlert(
        "Invalid Rule",
        ruleValidation.error || "Please review the regex rule.",
      );
      return;
    }

    const normalizedPattern =
      ruleValidation.normalizedPattern || effectivePattern.trim();
    const normalizedFlags = ruleValidation.normalizedFlags || "";
    const normalizedName =
      ruleDraft.name.trim() ||
      (ruleDraft.mode === "quick" && ruleDraft.quickConfig
