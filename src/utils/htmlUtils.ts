...
export const prepareTTSContent = (
  ...
  const $ = cheerio.load(html);
  const cleanupForDisplay = createRegexCleanupRunner(
    regexCleanupRules,
    "download",
  );
  ...
  elements.each((_, elem) => {
    ...
    if (
      $(elem).is("div") &&
      $(elem).children("p, div, h1, h2, h3, h4, h5, h6, ul, ol, blockquote")
        .length > 0
    ) {
      return;
    }
    ...
    if (
      ...
      currentChunkText.length + text.length > chunkSize
    ) {
...
export const extractFormattedText = (html: string): string => {
  ...
  $body.find("div").each((_, div) => {
    ...
    const allBlock =
      children.filter((_, child) => $(child).is(blockSelector)).length ===
      children.length;
    if (allBlock) {
...
export const cleanChapterTitle = (title: string): string => {
  ...
  let cleaned = title.replace(
    /\s*(?:[-–|]\s*)?\(?\s*(?:\d+|an?)\s+(?:second|minute|hour|day|week|month|year)s?\s+ago\s*\)?\s*$/i,
    "",
  );

  // Remove absolute dates if at the end, e.g. "Title Nov 25, 2025" or "Title - Nov 25, 2025"
  cleaned = cleaned.replace(
    /\s*(?:[-–|]\s*)?\(?\s*(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{1,2}(?:st|nd|rd|th)?,?\s+\d{4}|\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4})\s*\)?\s*$/i,
    "",
  );
