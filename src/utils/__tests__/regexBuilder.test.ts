...
describe('regexBuilder', () => {
    ...
    describe('generateQuickPattern', () => {
        ...
        it('should return empty for invalid input', () => {
            expect(generateQuickPattern({ characters: '', minCount: 3, wholeLine: false }).pattern).toBe('');
            expect(generateQuickPattern({ characters: '=', minCount: 0, wholeLine: false }).pattern).toBe('');
        });
    });

    describe('generateRuleName', () => {
        it('should generate descriptive name for single character', () => {
            const name = generateRuleName({ characters: '=', minCount: 5, wholeLine: true });
            expect(name).toBe('Remove = (5+) separator lines');
        });

        it('should generate name for inline pattern', () => {
            const name = generateRuleName({ characters: '-', minCount: 3, wholeLine: false });
            expect(name).toBe('Remove - (3+) patterns');
        });

        it('should truncate long character strings', () => {
            const name = generateRuleName({ characters: '####', minCount: 2, wholeLine: true });
        ...
        it('should return empty for no characters', () => {
            const name = generateRuleName({ characters: '', minCount: 3, wholeLine: true });
            expect(name).toBe('');
        });
    });
    ...
    describe('parseQuickPattern', () => {
        it('should parse simple single character pattern', () => {
            ...
            expect(result).toEqual({ characters: '=', minCount: 5, wholeLine: false });
        });
        ...
        it('should parse multi-character pattern', () => {
            ...
            expect(result).toEqual({ characters: '##', minCount: 4, wholeLine: false });
        });

        it('should parse multi-character whole line pattern', () => {
            const result = parseQuickPattern('^[\\s]*(?:\\*\\*){5,}[\\s]*$', 'gm');
            expect(result).toEqual({ characters: '**', minCount: 5, wholeLine: true });
        });

        it('should unescape special characters', () => {
            const result = parseQuickPattern('\\.{3,}', 'g');
            expect(result).toEqual({ characters: '.', minCount: 3, wholeLine: false });
        });
