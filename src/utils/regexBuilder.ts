...
export function generateQuickPattern(config: QuickBuilderConfig): {
  pattern: string;
  flags: string;
} {
...
export function generateRuleName(config: QuickBuilderConfig): string {
  ...
  const displayChars =
    characters.length > 4 ? `${characters.slice(0, 4)}...` : characters;
...
export function parseQuickPattern(
  pattern: string,
  flags: string,
): QuickBuilderConfig | null {
