...
describe("useLibrary", () => {
  ...
  it("should filter stories by author in search query", async () => {
    ...
    expect(result.current.stories.every((s) => s.author === "Author A")).toBe(
      true,
    );
  });
  ...
  it("should filter by source tag", async () => {
    ...
    expect(result.current.stories.length).toBe(2);
    expect(
      result.current.stories.every((s) => s.sourceUrl.includes("example.com")),
    ).toBe(true);
  });
  ...
  it("should filter by content tag", async () => {
    ...
    expect(result.current.stories.length).toBe(2);
    expect(
      result.current.stories.every((s) => s.tags?.includes("action")),
    ).toBe(true);
  });
