...
describe("useAddStory", () => {
  ...
  it("should show error when no chapters found", async () => {
    ...
    expect(mockShowAlert).toHaveBeenCalledWith(
      "Error",
      "No chapters found. Please check the URL.",
    );
  ...
  it("should handle fetch error", async () => {
    ...
    expect(mockShowAlert).toHaveBeenCalledWith(
      "Error",
      expect.stringContaining("Failed to fetch the novel"),
    );
    expect(result.current.loading).toBe(false);
    expect(result.current.statusMessage).toBe("");
  });
  ...
  it("should update status message during add process", async () => {
    mockProvider.getChapterList = jest
      .fn()
      .mockImplementation(async (_html, _url, callback) => {
