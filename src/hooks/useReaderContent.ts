...
export const useReaderContent = (storyId: string, chapterId: string) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (s) {
        ...
        const c =
          s.chapters.find((chap: Chapter) => chap.id === decodedChapterId) ||
      ...
      setError(
        e instanceof Error ? e.message : "Failed to load chapter content.",
      );
      setRedirectPath("/");
    } finally {
      setLoading(false);
  ...
  const isLastRead = Boolean(
    story && chapter && story.lastReadChapterId === chapter.id,
  );
