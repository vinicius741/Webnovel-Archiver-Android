...
import { epubGenerator } from "../../../services/epub/EpubGenerator";
import { Story } from "../../../types";

jest.mock("../../../services/storage/StorageService");
jest.mock("../../../services/epub/EpubGenerator");
...
describe("useStoryEPUB", () => {
  ...
  const baseStory: Story = {
    ...
    chapters: [
      ...
      },
      {
        id: "c5",
        title: "Chapter 5",
        url: "http://test.com/c5",
        downloaded: false,
      },
      {
        id: "c6",
        title: "Chapter 6",
        url: "http://test.com/c6",
        downloaded: true,
      },
    ],
    dateAdded: 1000,
  ...
  it("generates EPUBs using only downloaded chapters", async () => {
    ...
    expect(epubGenerator.generateEpubs).toHaveBeenCalledWith(
      baseStory,
      [
        baseStory.chapters[0],
        baseStory.chapters[2],
        baseStory.chapters[3],
        baseStory.chapters[5],
      ],
      150,
      expect.any(Function),
      [1, 3, 4, 6],
  ...
  it("applies start-after-bookmark when enabled", async () => {
    const allDownloadedChapters = baseStory.chapters.map((ch) => ({
      ...ch,
      downloaded: true,
    }));
    ...
    expect(epubGenerator.generateEpubs).toHaveBeenCalledWith(
      storyWithBookmark,
      [
        allDownloadedChapters[3],
        allDownloadedChapters[4],
        allDownloadedChapters[5],
      ],
      100,
      expect.any(Function),
      [4, 5, 6],
