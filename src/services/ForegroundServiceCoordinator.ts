...
const buildActions = () => {
  ...
  if (downloadActive && !ttsActive) {
    return [{ title: "Cancel", pressAction: { id: "cancel" } }];
  }
...
const updateForegroundNotification = async () => {
  ...
  try {
    ...
    console.warn(
      "[ForegroundServiceCoordinator] Failed to update notification:",
      e,
    );
  }
};
...
const stopForegroundIfIdle = async () => {
  ...
  try {
    ...
    console.warn(
      "[ForegroundServiceCoordinator] Failed to stop foreground service:",
      e,
    );
  }
};
...
export const registerForegroundService = async () => {
  ...
  notifee!.default.registerForegroundService(async () => {
    try {
      ...
      console.warn(
        "[ForegroundServiceCoordinator] Failed to start download manager in FGS:",
        e,
      );
    }
...
export const showDownloadCompletionNotification = async (
  title: string,
  body: string,
) => {
  if (!(await ensureInitialized())) return;
  try {
    ...
    console.warn(
      "[ForegroundServiceCoordinator] Failed to show completion notification:",
      e,
    );
  }
};
...
export const requestPermissions = async () => {
  ...
  try {
    ...
    console.warn(
      "[ForegroundServiceCoordinator] Failed to request permissions:",
      e,
    );
  }
};
