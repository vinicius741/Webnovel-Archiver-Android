...
import { EpubFileSystem } from "./EpubFileSystem";
...
export class EpubGenerator {
  ...
  async generateEpub(
    ...
    oebps.file(
      "toc.ncx",
      EpubMetadataGenerator.generateNcx(story, cleanChapters, uid),
    );
  ...
  async generateEpubs(
    ...
    const hasOriginalChapterNumbers =
      !!originalChapterNumbers &&
      originalChapterNumbers.length === chapters.length;
    ...
    for (let fileIndex = 0; fileIndex < totalFiles; fileIndex++) {
      ...
      const wrappedProgress = (progress: EpubProgress) => {
        ...
        const fileProgressWeight =
          (progress.current / progress.total) * (100 / totalFiles);
        const adjustedPercentage = Math.min(
          100,
          basePercentage + fileProgressWeight,
        );
