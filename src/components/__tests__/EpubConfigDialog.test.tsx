...
describe("EpubConfigDialog", () => {
  ...
  it("renders with initial values", () => {
    ...
    expect(
      getByText(
        "Downloaded chapters: 75. EPUB generation only includes downloaded chapters.",
      ),
    ).toBeTruthy();
  });
  ...
  it("shows validation error when max chapters is out of range", () => {
    ...
    expect(
      queryByText("Max chapters must be between 10 and 1000."),
    ).toBeTruthy();
  ...
  it("shows validation error when range is invalid", () => {
    ...
    expect(
      queryByText("Range start cannot be greater than range end."),
    ).toBeTruthy();
    expect(baseProps.onSave).not.toHaveBeenCalled();
  });
