  ...
  const MockIconButton = ({
    ...
    return React.createElement(
      ...
      React.createElement(
        Text,
        null,
        `${icon} ${disabled ? "(disabled)" : "(enabled)"}`,
      ),
    );
  };
...
describe("TTSController", () => {
  ...
  it("should not render when not visible", () => {
    const { queryByText } = renderWithTheme(
      <TTSController {...defaultProps} visible={false} />,
    );
  ...
  it("should show play icon when paused", () => {
    const { getByTestId } = renderWithTheme(
      <TTSController {...defaultProps} isPaused={true} />,
    );
  ...
  it("should show pause icon when not paused", () => {
    const { getByTestId } = renderWithTheme(
      <TTSController {...defaultProps} isPaused={false} />,
    );
