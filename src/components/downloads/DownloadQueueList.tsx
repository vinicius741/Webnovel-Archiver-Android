...
const getSubtitleText = (completedCount: number, totalCount: number, summary: StatusSummary): string => {
...
export const DownloadQueueList: React.FC<DownloadQueueListProps> = ({
    ...
    const handleStoryCancelAll = (jobs: DownloadJob[]) => {
        jobs.forEach(job => {
            if (job.status === 'pending' || job.status === 'paused' || job.status === 'downloading') {
                onCancel(job.id);
            }
        });
    };
    ...
    const renderStoryCard = ({ item }: { item: StoryGroup }) => {
        ...
        const completedCount = item.jobs.filter(j => j.status === 'completed').length;
        const totalCount = item.jobs.length;
        const hasFailed = summary.failed > 0;
        const isInProgress = summary.downloading > 0 || summary.pending > 0 || summary.paused > 0;
                    ...
                    <Card.Content style={styles.storyHeader}>
                        <View style={styles.storyTitleRow}>
                            ...
                            <View style={styles.storyInfo}>
                                <Text variant="titleMedium" numberOfLines={1} style={styles.storyTitle}>
                                    {item.storyTitle}
                                </Text>
                    ...
                    <View style={styles.chapterList}>
                        ...
                        {item.jobs.map((job, index) => (
                            <View key={job.id}>
                                ...
                                {index < item.jobs.length - 1 && <Divider style={styles.chapterDivider} />}
                            </View>
                        ))}
                    </View>
                )}
