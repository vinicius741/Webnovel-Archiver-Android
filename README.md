...
- **AI Features** — Per-novel AI-generated synopses and cover art through the user's own OpenRouter key: per-task model pickers (cheap text default; `x-ai/grok-imagine-image-2.0` for covers), preview-then-apply drafts, covers generated in one step or staged with an editable image prompt, background generation that survives navigation/app exit (foreground-service kept alive, drafts persisted, ready/failed notifications), and a show-AI/show-source switch for both descriptions and covers (the source text and cover URL are never modified; the AI cover shows across the app and embeds into EPUB exports while selected).
