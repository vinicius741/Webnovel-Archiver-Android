...
- **Multi-Source Support** — Extensible `SourceProvider` architecture with stable source descriptors for Royal Road, Scribble Hub, SpaceBattles, FanFiction.net, and Archive of Our Own (AO3). Providers own URL matching, fetching, parsing, network/browser policy, capabilities, and featured metadata. SpaceBattles imports use main Threadmarks and Reader pages, excluding forum replies and optional threadmark categories. FanFiction.net imports accept any chapter URL and discover the complete work from its chapter selector. AO3 imports accept work URLs and work/chapter URLs, use the full chapter index for updates, and preserve fandoms, warnings, tags, and engagement metrics. Public works are supported; login-restricted works currently report an error.
    ...
    src/
      main/
        java/com/vinicius741/webnovelarchiver/
          ...
          ai/                  # OpenRouter + TypeSafe clients, AI engines and background jobs
